...
const POWER_MARK = Object.freeze({
  crown: "👑",
  devil: "😈",
  cash: "💸",
  clock: "⏳",
});
const POWER_NAME = Object.freeze({
  crown: "Coroa",
  devil: "Diabinho",
  cash: "Grana extra",
  clock: "Congela-tempo",
...
const SPECIAL_MARK = Object.freeze({
  row: "↔",
  col: "↕",
  bomb: "💣",
  prism: "◆",
});
const SPECIAL_NAME = Object.freeze({
  row: "Seta horizontal",
  col: "Seta vertical",
  bomb: "Bomba 3×3",
  prism: "Arco-íris",
...
const HUD_REWARD = Object.freeze({
  read: 360,
  approach: 980,
  land: 1120,
  arrival: 1300,
...
function heldHudAmount(side, kind) {
  ...
  for (const hold of hudHolds) {
    if (hold.match !== state || hold.side !== side || hold.kind !== kind)
      continue;
    if (
      kind === "moves" &&
      (state.currentSide !== side || hold.round !== state.currentRound)
    )
      continue;
    total += hold.amount;
  }
...
function queueHudReward(
  ...
  const destination = {
    ...
    time: "Clock",
    moves: "Moves",
    crown: "CrownBoostTime",
    penalty: "Clock",
  }[kind];
  const laneId = `${side}${destination}Reward`;
  const node = $(laneId);
  if (!node) {
    if (hold) hudHolds.delete(hold);
    return;
  }
  ...
  queue.push({
    reservation,
    text: hudRewardText(side, kind, amount, label),
    customLabel: !!label,
    side,
    kind,
    destination,
    ...
    (kind === "score"
      ...
      : kind === "cash"
        ...
        : kind === "moves"
          ? `+${amount} MOV.`
          : kind === "crown"
            ? `👑 x${crownMultiplierFor(side)}`
            : amount < 0
              ? `😈 −${Number(Math.abs(amount).toFixed(1)).toLocaleString("pt-BR")}s`
              : `+${Number(amount.toFixed(1)).toLocaleString("pt-BR")}s`)
  );
}

function rewardOrigin(positions) {
  const cells = [...positions].map((pos) =>
    typeof pos === "string" ? parseKey(pos) : pos,
  );
  if (!cells.length) return null;
  return {
    x: (cells.reduce((sum, pos) => sum + pos.c, 0) / cells.length + 0.5) / SIZE,
    y: (cells.reduce((sum, pos) => sum + pos.r, 0) / cells.length + 0.5) / SIZE,
  };
}
...
function startRewardFlight(job, target, slot) {
  ...
  const paint = (now) => {
    ...
    if (revision !== rewardGeometryRevision) {
      ...
      const endRect = targetRect.width
        ? targetRect
      ...
      const sourceY = clamp(
        sourceRect.top + sourceRect.height * (job.origin?.y ?? 0.35),
        marginY,
        innerHeight - marginY,
      );
      ...
      from = {
        x: clamp(
          sourceRect.left + sourceRect.width * (job.origin?.x ?? 0.5),
          marginX,
          innerWidth - marginX,
        ),
        y: clamp(
          sourceY + job.stack * 46 * stackDirection,
          marginY,
          innerHeight - marginY,
        ),
      };
      ...
      const gap = Math.min(48, distance * 0.2);
      approach = {
        x: to.x + ((from.x - to.x) / distance) * gap,
        y: to.y + ((from.y - to.y) / distance) * gap,
      };
    ...
    if (elapsed >= HUD_REWARD.read && elapsed < HUD_REWARD.approach) {
      const t =
        (elapsed - HUD_REWARD.read) / (HUD_REWARD.approach - HUD_REWARD.read);
      ...
      const t = clamp(
        (elapsed - HUD_REWARD.land) / (HUD_REWARD.arrival - HUD_REWARD.land),
        0,
        1,
      );
...
function runHudReward(laneId) {
  ...
  hudLater(() => {
    ...
    node.classList.toggle(
      "attack",
      job.kind === "penalty" || job.reservation.amount < 0,
    );
    ...
    if (job.kind === "cash" && job.reservation.amount > 0)
      playPowerSound("cash", job.side);
    if (!flight) node.classList.add("visible"); // Reduced motion still gets reading time.
    hudLater(() => {
      removeRewardFlight(flight);
      node.classList.add("visible");
      pulseHud(
        target.getClientRects().length ? target : node,
        job.kind === "penalty" || job.reservation.amount < 0,
      );
    }, HUD_REWARD.arrival);
    hudLater(() => {
...
function resetHudRewards() {
  ...
  energyBatches.clear();
  document
    .querySelectorAll(".celebrating-creation, .moment-source, .moment-target")
    .forEach((node) => {
...
function remainingHudRewardTime() {
  ...
  for (const batch of energyBatches)
    remaining = Math.max(remaining, batch.endAt - performance.now());
  for (const queue of hudLanes.values()) {
    const queued = queue
      .slice(1)
      .reduce((sum, job) => sum + job.wait + HUD_REWARD.end, 0);
    remaining = Math.max(
      remaining,
      queue[0].endAt - performance.now() + queued,
    );
  }
...
function energyLabel(side, kind) {
  ...
  if (attack) {
    setHudText(
      node,
      (amount ? hudRewardText(side, kind, amount) + " · " : "") + attack.text,
    );
...
function settleEnergyBatch(batch) {
  if (!batch.closed || batch.pending || batch.settling || batch.match !== state)
    return;
  batch.settling = true;
...
function addEnergyPart(
  ...
  batch.pending += 1;
  batch.endAt = Math.max(
    batch.endAt,
    performance.now() + wait + ENERGY.travel + ENERGY.end,
  );
  hudLater(() => {
...
function startEnergyFlight(side, kind, origin) {
  ...
  if (energyFrame === null)
    energyFrame = requestAnimationFrame(paintEnergyFlights);
  return flight;
}
...
function paintEnergyFlights(now) {
  ...
  for (const f of energyFlights) {
    ...
    const a = rect(boardRoot(f.side)),
      b = rect($(f.side + energyDestination(f.kind)));
    const clamp = (n, max) => Math.max(10, Math.min(max - 10, n));
    f.from = {
      x: clamp(a.left + a.width * (f.origin?.x ?? 0.5), innerWidth),
      y: clamp(a.top + a.height * (f.origin?.y ?? 0.4), innerHeight),
    };
    f.to = {
      x: clamp(b.left + b.width / 2, innerWidth),
      y: clamp(b.top + b.height / 2, innerHeight),
    };
    f.control = {
      x: clamp(
        (f.from.x + f.to.x) / 2 +
          (f.side === "player" ? -1 : 1) *
            Math.min(90, Math.abs(f.from.y - f.to.y) * 0.2),
        innerWidth,
      ),
      y: (f.from.y + f.to.y) / 2,
    };
...
async function ensureFirebase() {
  ...
  if (!firebaseLoadPromise) {
    firebaseLoadPromise = import("./firebase.js")
      .then((mod) => {
        db = mod.db;
        fbDoc = mod.doc;
        fbGetDoc = mod.getDoc;
  ...
  if (!powerAudioLoads.has(kind)) {
    powerAudioLoads.set(
      ...
      fetch(POWER_AUDIO[kind].src)
        ...
        .then((buffer) => {
          powerAudioDurations.set(kind, buffer.duration);
          return buffer;
        })
        .catch((error) => {
          console.warn(`[Áudio] Poder ${kind} indisponível.`, error);
          return null;
        }),
    );
  }
...
function pumpPowerAudio() {
  ...
  for (let i = powerAudioQueue.length - 1; i >= 0; i--) {
    ...
    if (job.match !== state || now - job.created > 2600)
      powerAudioQueue.splice(i, 1);
  }
  if (!powerAudioQueue.length) return;
  powerAudioQueue.sort(
    (a, b) => b.priority - a.priority || a.created - b.created,
  );
  ...
  if (job.priority >= 2.5) {
    for (const active of activePowerAudio) {
      ...
      if (
        ...
        (activePowerAudio.size >= 2 ||
          (POWER_AUDIO[job.kind].voice && POWER_AUDIO[active.kind].voice))
      )
        stopPowerAudio(active);
    }
  }
  const voiceBusy =
    POWER_AUDIO[job.kind].voice &&
    [...activePowerAudio].some((active) => POWER_AUDIO[active.kind].voice);
  ...
  const volume =
    config.volume * (job.side === "rival" && job.kind !== "devil" ? 0.35 : 1);
  ...
  if (!preserveEnvelope) {
    ...
    gain.gain.setValueAtTime(
      volume,
      startAt + Math.max(0.025, duration - 0.16),
    );
...
async function playPowerSound(
  kind,
  side,
  { distinct = false, spotlight = false } = {},
) {
  const match = state;
  ...
  if (
    !buffer ||
    state !== match ||
    epoch !== powerAudioEpoch ||
    document.hidden ||
    performance.now() - created > 2600
  )
    return;
  // Coalesce repeated powers in a cascade, including while a clip is playing.
  if (
    !distinct &&
    [...activePowerAudio, ...powerAudioQueue].some(
      (job) => job.kind === kind && job.side === side,
    )
  )
    return;
  const priority =
    kind === "devil" && side === "rival"
      ? 3
      : side === "player"
        ? spotlight
          ? 2.5
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) resetPowerAudio();
});
...
function ensureBombAudio() {
  ...
  if (!bombAudioLoadPromise) {
    bombAudioLoadPromise = fetch(BOMB_AUDIO_SRC)
      .then((response) => {
        if (!response.ok)
          throw new Error(`Falha ao carregar explosão: ${response.status}`);
        return response.arrayBuffer();
      })
...
async function playMatchSound(side, cascade = 1) {
  ...
  const rateIndex = Math.min(
    MATCH_AUDIO_RATE.length - 1,
    Math.max(0, cascade - 1),
  );
  source.buffer = buffer;
...
function makeCell(
  ...
  return {
    id: nextCellId++,
    type: Number.isInteger(powerType) ? powerType : type,
    special,
    power,
  };
}
...
function makesInitialMatch(board, r, c, type) {
  if (
    c >= 2 &&
    matchType(board[r][c - 1]) === type &&
    matchType(board[r][c - 2]) === type
  )
    return true;
  if (
    r >= 2 &&
    matchType(board[r - 1][c]) === type &&
    matchType(board[r - 2][c]) === type
  )
    return true;
  return false;
}
...
function createBoard() {
  ...
  for (let r = 0; r < SIZE; r++) {
    for (let c = 0; c < SIZE; c++) {
      let cell;
      do cell = makeCell();
      while (makesInitialMatch(board, r, c, matchType(cell)));
      board[r][c] = cell;
    }
  }
...
function findMatches(board) {
  ...
  for (let r = 0; r < SIZE; r++) {
    ...
    for (let c = 1; c <= SIZE; c++) {
      ...
      if (c < SIZE && startType != null && matchType(board[r][c]) === startType)
        continue;
      const len = c - start;
  ...
  for (let c = 0; c < SIZE; c++) {
    ...
    for (let r = 1; r <= SIZE; r++) {
      ...
      if (r < SIZE && startType != null && matchType(board[r][c]) === startType)
        continue;
      const len = r - start;
      if (startType != null && len >= 3) {
        const cells = [];
...
function scoreGroups(groups, cascade) {
  ...
  for (const group of groups) {
    const base =
      SCORE_BY_SIZE[Math.min(5, group.length)] || 120 + (group.length - 5) * 50;
    score += base;
  }
...
function buildSpecialCreations(board, matchResult, preferred = []) {
  ...
  for (const [coord, indexes] of memberships.entries()) {
    ...
    const intersect = rows.find((row) =>
      cols.some((col) => col.type === row.type),
    );
    if (!intersect) continue;
    const pos = parseKey(coord);
    const involved = indexes.filter(
      (index) => matchResult.groups[index].type === intersect.type,
    );
    involved.forEach((index) => tOrLGroups.add(index));
    const sources = [];
    involved.forEach((index) => {
      matchResult.groups[index].cells.forEach((cell) => {
        if (!sources.some((source) => samePos(source, cell)))
          sources.push({ r: cell.r, c: cell.c });
      });
    });
    creations.set(coord, {
      pos,
  ...
  matchResult.groups.forEach((group, index) => {
    ...
    if (group.length >= 5) special = "prism";
      ...
      special = group.orientation === "row" ? "row" : "col";
    if (!special) return;
    ...
    if (
      !existing ||
      SPECIAL_PRIORITY[special] > SPECIAL_PRIORITY[existing.special]
    ) {
      creations.set(k, {
        pos,
        special,
...
function expandAllEffects(board, initial, protectedKeys = new Set()) {
  ...
  for (const value of protectedKeys) {
    if (initial.has(value) && !queued.has(value)) {
      queue.push(value);
      queued.add(value);
    }
  }
  ...
  const enqueueNew = (before) => {
    for (const value of clearSet) {
      if (!before.has(value) && !queued.has(value)) {
        queue.push(value);
        queued.add(value);
      }
    }
    ...
    if (cell.special && !triggered.has(currentKey)) {
      ...
      if (cell.special === "row") addRow(clearSet, pos.r);
        ...
        const neighbors = [
          ...
          .filter(Boolean)
          .filter((neighbor) => neighbor.special !== "prism");
...
function buildDirectSpecialPlan(board, a, b) {
  ...
  if (cellA.special === "prism" || cellB.special === "prism") {
    ...
    if (other?.special === "prism") {
      for (let r = 0; r < SIZE; r++)
        for (let c = 0; c < SIZE; c++) clearSet.add(key(r, c));
      ...
      if (other?.special === "row") {
        ...
        for (let r = 0; r < SIZE; r++)
          for (let c = 0; c < SIZE; c++) {
            if (matchType(board[r][c]) === other.type)
              addArea(clearSet, { r, c }, 1);
          }
      }
    }
    ...
    if (types.has("bomb") && types.size === 1) {
      ...
      if (lineCell.special === "row") {
        for (
          let r = Math.max(0, bombPos.r - 1);
          r <= Math.min(SIZE - 1, bombPos.r + 1);
          r++
        )
          addRow(clearSet, r);
      } else if (lineCell.special === "col") {
        for (
          let c = Math.max(0, bombPos.c - 1);
          c <= Math.min(SIZE - 1, bombPos.c + 1);
          c++
        )
          addCol(clearSet, c);
      }
...
function reshuffleBoard(board) {
  ...
  for (let attempt = 0; attempt < 220; attempt++) {
    ...
    const candidate = Array.from({ length: SIZE }, (_, r) =>
      shuffled.slice(r * SIZE, (r + 1) * SIZE),
    );
...
async function ensurePlayableForSide(side, { announce = true } = {}) {
  ...
  if (announce) {
    ...
    const stateNode = $(
      side === "player" ? "playerMoveState" : "rivalMoveState",
    );
    if (stateNode) stateNode.textContent = "Sem movimentos";
    if (side === "player")
      $("comboLabel").textContent =
        "Sem movimentos possíveis. Embaralhando automaticamente…";
    await delay(620);
  }
...
function bonusMoneyFor(side) {
  ...
  return (
    Number(
      side === "player" ? state.playerBonusMoney : state.rivalBonusMoney,
    ) || 0
  );
}
...
function formatMoney(value) {
  return `R$ ${Math.max(0, Number(value) || 0)
    .toFixed(2)
    .replace(".", ",")}`;
}
...
function crownBoostFor(side) {
  ...
  return Math.max(
    0,
    Number(
      side === "player" ? state.playerCrownBoost : state.rivalCrownBoost,
    ) || 0,
  );
}
...
function crownLevelFor(side) {
  ...
  return Math.max(
    ...
    Math.floor(
      Number(
        side === "player" ? state.playerCrownLevel : state.rivalCrownLevel,
      ) || 0,
    ),
  );
}
...
function activateCrownBoost(side, count = 1, origin = null) {
  ...
  queueHudReward(side, "crown", Math.max(1, Math.floor(Number(count) || 1)), {
    origin,
  });
...
function renderCrownBoost(side) {
  ...
  const timeNode = $(
    side === "player" ? "playerCrownBoostTime" : "rivalCrownBoostTime",
  );
  const fill = $(
    side === "player" ? "playerCrownBoostFill" : "rivalCrownBoostFill",
  );
...
function updateMoneyLeadDisplay(
  playerShown = scoreDisplay.player,
  rivalShown = scoreDisplay.rival,
) {
  if (!state) return;
  ...
  for (const [side, score] of [
    ...
    totals[side] = Math.max(
      0,
      totalMoneyFor(side, score) - heldHudAmount(side, "cash"),
    );
  ...
  const label = solo
    ...
    : diff === 0
      ? "EMPATE"
      : sideName(diff > 0 ? "player" : "rival").toUpperCase() + " NA FRENTE";
  setHudText($("moneyLeadLabel"), label);
  setHudText(
    $("moneyLeadValue"),
    (solo || !diff ? "" : "+") +
      formatMoney(solo ? totals.player : Math.abs(diff)),
  );
}
...
function animateScoreHud(side, target) {
  ...
  if (
    ...
    Math.round(scoreDisplay[side]) === safeTarget
  ) {
    setHudText(node, safeTarget.toLocaleString("pt-BR"));
    return;
  }

  if (scoreAnimationFrame[side])
    cancelAnimationFrame(scoreAnimationFrame[side]);
...
function showDevilAttack(
  side,
  penaltySeconds,
  appliedSeconds = penaltySeconds,
  origin = null,
  wait = 0,
) {
  ...
  queueHudReward(
    target,
    turns ? "penalty" : "time",
    turns ? appliedSeconds : -appliedSeconds,
    {
      sourceSide: side,
      origin,
      wait,
      ...
      label:
        appliedSeconds > 0
          ? "😈 −" +
            Number(appliedSeconds.toFixed(1)).toLocaleString("pt-BR") +
            "s" +
            (turns ? " PRÓX." : "")
          : turns
            ? "😈 LIMITE"
...
function renderTimeBars() {
  ...
  [
    ["player", "playerTimeBar", "playerTimeFill"],
    ["rival", "rivalTimeBar", "rivalTimeFill"],
  ].forEach(([side, barId, fillId]) => {
    const bar = $(barId);
    const fill = $(fillId);
    if (!bar || !fill) return;
    ...
    if (race) {
      ...
      const overflowPct = Math.max(
        0,
        Math.min(55, ((current - base) / base) * 100),
      );
    ...
    const visible =
      ...
      (side === "player" || state.opponentType !== "solo");
...
function showPlayFeedback(
  ...
  for (const [kind, amount] of [
    ...
    addEnergyPart(
      ...
      amount,
      origin,
      holds[kind] || holdHudChange(side, kind, amount),
      wait,
    );
    wait += ENERGY.stagger;
  }
  if (rewards.presentation) {
    rewards.presentation.closed = true;
    settleEnergyBatch(rewards.presentation);
  }
}
...
function flushCascadeFeedback(side, rewards) {
  ...
  for (const [kind, total] of [
    ...
    addEnergyPart(
      ...
      kind,
      total - already,
      rewards.stepOrigin || rewards.origin,
      rewards.holds[kind],
      wait,
    );
    wait += ENERGY.stagger;
...
function renderHud() {
  ...
  animateScoreHud(
    "player",
    state.playerScore - heldHudAmount("player", "score"),
  );
  ...
  $("rivalBoard").classList.toggle(
    "bot-controlled",
    state.opponentType === "bot",
  );

  if (state.format === "race") {
    setHudText(
      $("centerLabel"),
      state.opponentType === "solo" ? "PARTIDA RÁPIDA" : "CORRIDA",
    );
    setHudText(
      $("centerValue"),
      state.opponentType === "solo" ? "TEMPO" : "AO VIVO",
    );
    setHudText(
      $("centerSub"),
      state.opponentType === "solo"
    ...
    setHudText(
      $("centerLabel"),
      `RODADA ${Math.min(state.currentRound, state.rounds)}/${state.rounds}`,
    );
    setHudText(
      $("centerValue"),
      activeSide === "player" ? "VOCÊ" : sideName("rival").toUpperCase(),
    );
    ...
    setHudText(
      $("playerClock"),
      activeSide === "player"
        ? formatSeconds(turnTime)
        : formatSeconds(
            TURN_SECONDS -
              turnPenaltyFor("player") +
              heldHudAmount("player", "penalty"),
          ),
    );
    setHudText(
      $("rivalClock"),
      activeSide === "rival"
        ...
        : formatSeconds(
            TURN_SECONDS -
              turnPenaltyFor("rival") +
              heldHudAmount("rival", "penalty"),
          ),
    );
  }
...
function updatePanelStates() {
  ...
  if (state.format === "race") {
    ...
    setHudClass(
      $("playerMoveState"),
      `move-state${busy.player ? " playing" : state.playerTime > 0 ? "" : " waiting"}`,
    );
    if (state.opponentType === "online") {
      setHudText(
        $("rivalMoveState"),
        state.rivalTime > 0 ? "Jogando" : "Tempo acabou",
      );
      setHudClass(
        $("rivalMoveState"),
        `move-state${state.rivalTime > 0 ? "" : " waiting"}`,
      );
    } else if (state.opponentType === "bot") {
      if (!busy.rival) {
        setHudText(
          $("rivalMoveState"),
          state.rivalTime > 0 ? "Pensando…" : "Tempo acabou",
        );
        setHudClass(
          $("rivalMoveState"),
          `move-state${state.rivalTime > 0 ? " thinking" : " waiting"}`,
        );
      }
    }
  } else {
    ...
    setHudClass(
      $("playerMoveState"),
      `move-state${state.currentSide === "player" ? (busy.player ? " playing" : "") : " waiting"}`,
    );
    if (state.opponentType === "online") {
      setHudText(
        $("rivalMoveState"),
        state.currentSide === "rival" ? "Jogando" : "Aguardando",
      );
      setHudClass(
        $("rivalMoveState"),
        `move-state${state.currentSide === "rival" ? " playing" : " waiting"}`,
      );
    } else if (state.opponentType === "bot") {
      if (!busy.rival) {
        setHudText(
          $("rivalMoveState"),
          state.currentSide === "rival" ? "Pensando…" : "Aguardando",
        );
        setHudClass(
          $("rivalMoveState"),
          `move-state${state.currentSide === "rival" ? " thinking" : " waiting"}`,
        );
      }
    }
  }
    ...
    row.forEach((cell, c) => {
      ...
      if (button && button._signature === signature) {
        ...
        button.classList.toggle(
          "selected",
          !!selected[side] && selected[side].r === r && selected[side].c === c,
        );
      ...
      const gemMeta =
        GEM_TYPES[Number.isInteger(effectiveType) ? effectiveType : cell.type];
      const specialName = cell.special
        ? `, especial ${SPECIAL_NAME[cell.special]}`
        : "";
      const powerName = cell.power ? `, poder ${POWER_NAME[cell.power]}` : "";
      button.setAttribute(
        "aria-label",
        `Peça ${gemMeta.name} ${gemMeta.icon}${specialName}${powerName}, linha ${r + 1}, coluna ${c + 1}`,
      );
      ...
      if (selected[side] && selected[side].r === r && selected[side].c === c)
        button.classList.add("selected");
      if (cell.special) button.classList.add(`special-${cell.special}`);
      if (cell.power)
        button.classList.add("random-power", `power-${cell.power}`);
      ...
      if (cell.power && cell.special && cell.special !== "prism")
        button.classList.add("stacked-marks");
      next.set(cell.id, button);
    }),
  );
...
function getCellElement(side, pos) {
  return (
    boardRoot(side)?.querySelector(
      `.gem[data-r="${pos.r}"][data-c="${pos.c}"]`,
    ) || null
  );
}
...
async function animateSwap(side, a, b, { invalid = false } = {}) {
  ...
  const options = {
    duration,
    easing: invalid ? "ease-in-out" : "cubic-bezier(.2,.85,.3,1)",
    fill: "both",
  };
  const animations = [
    first.animate(firstFrames, options),
    second.animate(secondFrames, options),
  ];
...
async function animateFalls(side, movement) {
  ...
  movement.forEach((info, id) => {
    ...
    const stagger = info.isNew
      ? Math.min(110, Math.max(0, (-info.fromR - 1) * 22))
      : 0;
    animations.push(
      el.animate(
        [
          {
            ...
            opacity: info.isNew ? 0.12 : 1,
            offset: 0,
          },
          { transform: "translateY(3px) scale(1)", opacity: 1, offset: 0.88 },
          { transform: "translateY(0) scale(1)", opacity: 1, offset: 1 },
        ],
        {
          duration,
          delay: stagger,
          easing: "cubic-bezier(.16,.76,.24,1)",
          fill: "both",
        },
      ),
    );
...
function triggeredLineEffects(board, triggered) {
  ...
  triggered.forEach((value) => {
    ...
    if (cell.special === "row" || cell.special === "col")
      list.push({ r: pos.r, c: pos.c, special: cell.special });
  });
  return list;
}
...
async function showLineEffects(side, effects) {
  ...
  geometry.forEach(({ effect, rect }) => {
    ...
    if (effect.special === "row") {
      ...
      blast.style.transformOrigin = `${((effect.c + 0.5) / SIZE) * 100}% 50%`;
    } else {
      blast.style.left = `${rect.left - rootRect.left + rect.width / 2 - 8}px`;
      blast.style.top = "0px";
      blast.style.height = `${rootRect.height}px`;
      blast.style.transformOrigin = `50% ${((effect.r + 0.5) / SIZE) * 100}%`;
    }
    root.appendChild(blast);
    const botLite = side === "rival" && state?.opponentType === "bot";
    if (!reduced && blast.animate) {
      const peakOpacity = botLite ? 0.34 : 1;
      const frames =
        effect.special === "row"
          ? [
              { transform: "scaleX(.08)", opacity: 0 },
              { transform: "scaleX(1)", opacity: peakOpacity, offset: 0.25 },
              { transform: "scaleX(1)", opacity: 0 },
            ]
          : [
              { transform: "scaleY(.08)", opacity: 0 },
              { transform: "scaleY(1)", opacity: peakOpacity, offset: 0.25 },
              { transform: "scaleY(1)", opacity: 0 },
            ];
      waits.push(
        blast
          .animate(frames, {
            duration: botLite ? 220 : 360,
...
async function previewSpecialCreations(side, creations) {
  ...
  const geometry = creations.map((creation) => {
    ...
    return {
      ...
      sources: (creation.sources || []).map((source) => {
  ...
  geometry.forEach(({ creation, targetEl, targetRect, sources }) => {
    ...
    sources.forEach(({ source, sourceEl, rect }) => {
      const isTarget =
        source.r === creation.pos.r && source.c === creation.pos.c;
      if (!sourceEl) return;
      if (isTarget) {
        ...
        animations.push(
          sourceEl.animate(
            [
              ...
              {
                transform: `translate(${dx * 0.62}px, ${dy * 0.62}px) scale(.78)`,
                opacity: 0.86,
              },
              {
                transform: `translate(${dx}px, ${dy}px) scale(.38)`,
                opacity: 0,
              },
            ],
            {
...
async function showCreationMoments(side, creations, rewards) {
  ...
  try {
    for (const creation of creations) {
      ...
      const moves =
        match.format === "turns" ? turnMovesFromCreation([creation]) : 0;
      const duration = moves
        ? Math.max(
            CREATION_MOMENT.moves,
            Math.min(
              POWER_AUDIO.moves.duration,
              powerAudioDurations.get("moves") || POWER_AUDIO.moves.duration,
            ) *
              1000 +
              80,
          )
        : CREATION_MOMENT.special;
      const sources = (creation.sources || [creation.pos])
        .map((pos) => getCellElement(side, pos))
        .filter(Boolean);
      ...
      if (!moves || creation.special === "prism")
        void playPowerSound("crown", side, { distinct: true, spotlight: true });
  ...
  return () => {
    ...
    for (const rect of origins) {
      ...
      for (let index = 0; index < 8; index += 1) {
        ...
        spark.className = "bomb-impact-spark";
        impact.append(spark);
...
async function showClear(side, clearSet, triggered = new Set()) {
  const root = boardRoot(side);
  const match = state;
  const startBombImpact = prepareBombImpact(side, triggered);
  const cells = [];
  for (const value of clearSet) {
...
function turnMovesFromCreation(creations) {
  return creations.filter((creation) =>
    ["row", "col", "bomb", "prism"].includes(creation.special),
  ).length;
}
...
function clockFreezeFor(side) {
  ...
  return Math.max(
    0,
    Number(
      side === "player" ? state.playerClockFreeze : state.rivalClockFreeze,
    ) || 0,
  );
}
...
function addClockFreeze(side, count = 1) {
  ...
  setClockFreeze(
    side,
    clockFreezeFor(side) +
      Math.max(0, Number(count) || 0) * CLOCK_FREEZE_SECONDS,
  );
...
function turnPenaltyFor(side) {
  ...
  return Math.max(
    0,
    Number(
      side === "player" ? state.playerTurnPenalty : state.rivalTurnPenalty,
    ) || 0,
  );
}
...
function publishOnlineRaceDevilPenalty(targetTime) {
  ...
  void fbUpdateDoc(ref, {
    ...
    [`match.players.${onlineSeat}.powerAudioEvents`]: {
      ...state.powerAudioEvents,
    },
    updatedAt: Date.now(),
  }).catch((error) =>
    console.error("[Online] Falha ao publicar Diabinho:", error),
  );
}
...
function applyDevilEffect(side, count = 0) {
  ...
  if (state.format === "race") {
    const penalty =
      Math.max(0, Number(count) || 0) * DEVIL_RACE_PENALTY_SECONDS;
    ...
    if (state.opponentType === "online" && side === "player")
      publishOnlineRaceDevilPenalty(timeFor(target));
    return penalty;
  }
...
function finalizeCascadeRewards(side, rewards) {
  ...
  if (state.format === "race" && rewards.seconds > 0)
    awardTime(side, rewards.seconds);
  if (state.format === "turns" && rewards.moves > 0) {
    state.movesLeft += rewards.moves;
    if (rewards.moves > (rewards.announcedMoves || 0))
      recordPowerAudio("moves", side);
  }
  if (rewards.clockCount > 0) addClockFreeze(side, rewards.clockCount);
  const target = otherSide(side);
  const before =
    state.format === "race" ? timeFor(target) : turnPenaltyFor(target);
  ...
  if (side === "player")
    $("comboLabel").textContent = "Arraste uma joia para combinar.";
  renderHud();
}
...
async function resolveMatches(side, preferred = [], sharedRewards = null) {
  ...
  while (state === match && state && !state.finished) {
    ...
    const creations = buildSpecialCreations(
      board,
      result,
      cascade === 1 ? preferred : [],
    );
    const protectedKeys = new Set(
      creations.map((creation) => key(creation.pos.r, creation.pos.c)),
    );
    const expanded = expandAllEffects(board, result.matched, protectedKeys);
    const clearSet = expanded.clearSet;
    const extraClears = Math.max(
      0,
      clearSet.size - Math.max(0, result.matched.size - protectedKeys.size),
    );
    const rawGained =
      scoreGroups(result.groups, cascade) +
      extraClears * 12 +
      creations.length * 40;
    const origin = rewardOrigin(
      cascade === 1 && preferred.length ? preferred : result.matched,
    );
    ...
    const moneyBonus =
      Math.round((expanded.cashCount || 0) * DIRECT_CASH_BONUS * 100) / 100;
    if (moneyBonus) addBonusMoney(side, moneyBonus);

    if (state.format === "race")
      rewards.seconds += timeBonusFromCreation(creations, cascade);
    ...
    if (side === "player")
      $("comboLabel").textContent =
        cascade > 1
          ? `Resolvendo cascata ×${cascade}…`
          : "Resolvendo combinação…";
    ...
    if (stateNode)
      stateNode.textContent =
        cascade > 1 ? `Cascata x${cascade}` : `+${gained}`;
    renderHud();
    ...
    if (state !== match || match.finished) return total;
    await showLineEffects(
      side,
      triggeredLineEffects(board, expanded.triggered),
    );
    if (state !== match || match.finished) return total;
...
async function resolveDirectSpecialSwap(side, a, b) {
  ...
  const rawGained = Math.round(
    90 + plan.clearSet.size * 14 + plan.triggered.size * 35,
  );
  ...
  const moneyBonus =
    Math.round((plan.cashCount || 0) * DIRECT_CASH_BONUS * 100) / 100;
  if (moneyBonus) addBonusMoney(side, moneyBonus);
...
async function performSwap(side, a, b, { automated = false } = {}) {
  ...
  if (state.format === "turns" && (state.turnTimeLeft <= 0 || state.turnEnding))
    return false;
  const match = state;
  ...
  if (!valid) {
    if (side === "player")
      $("comboLabel").textContent = "Essa troca não forma combinação.";
    busy[side] = false;
    renderHud();
    return false;
  }
...
function evaluateBotMove(board, move) {
  ...
  if (isDirectSpecialCombo(aCell, bCell)) {
    ...
    return (
      500 + buildDirectSpecialPlan(test, move.a, move.b).clearSet.size * 20
    );
  }
  swap(test, move.a, move.b);
  ...
  return (
    scoreGroups(result.groups, 1) +
    creations.reduce((sum, c) => sum + SPECIAL_PRIORITY[c.special] * 100, 0)
  );
}
...
async function botMoveOnce(token) {
  ...
  const scored = moves
    .map((move) => ({
      move,
      immediate: evaluateBotMove(state.rivalBoard, move),
    }))
  ...
  if (!state || state.finished || token !== botTurnToken) {
    busy.rival = false;
    return false;
  }
...
async function endTurn(side, { timeout = false } = {}) {
  ...
  if (timeout) {
    ...
    if (side === "player")
      $("comboLabel").textContent = "⏱️ Tempo do turno acabou.";
  }
  ...
  state.turnEnding = false;
  renderAllBoards();
  if (state.opponentType === "bot" && state.currentSide === "rival")
    void runBotTurn();
}
...
function tickTurnMode() {
  ...
  if (!remoteWaiting && !busy[side] && !state.turnEnding) {
    ...
    if (activeDt > 0)
      state.turnTimeLeft = Math.max(0, state.turnTimeLeft - activeDt);
  }
...
function tickRace() {
  ...
  if (state.playerTime > 0) {
    ...
    if (activeDt > 0)
      state.playerTime = Math.max(0, state.playerTime - activeDt);
  }
...
function onlineRoomRef() {
  return onlineRoomId
    ? fbDoc(db, "buracoGames", `jewels-${onlineRoomId}`)
    : null;
}
...
async function invitePlayer() {
  ...
  const data = {
    title: "Joias Findom",
    text: "Vem jogar Joias Findom comigo!",
    url: roomShareUrl(),
  };
  ...
  try {
    ...
    try {
      if (supported && navigator.canShare) supported = navigator.canShare(data);
    } catch {
      supported = false;
    }
    if (supported) {
  ...
  try {
    ...
    if (startText && $("opponentSelect").value === "online") {
      startText.textContent =
        Number.isInteger(onlineSeat) && ready[onlineSeat]
          ? "CANCELAR PRONTO"
          : "FICAR PRONTO";
    }
  } finally {
  ...
  const match = {
    ...
    players: {
      0: {
        ...
        score: 0,
        bonusMoney: 0,
        time: Number(config.duration || 60),
        crownBoost: 0,
        crownLevel: 0,
        clockFreeze: 0,
        turnPenalty: 0,
...
function startOnlineGameFromSnapshot(match, lobby) {
  ...
  $("comboLabel").textContent =
    state.format === "race"
      ...
      : state.currentSide === "player"
        ? `Sua vez: ${state.movesLeft} movimentos.`
        : `Vez de ${sideName("rival")}.`;
...
function syncOnlineGameFromSnapshot(match, lobby) {
  if (
    !state ||
    state.opponentType !== "online" ||
    state.onlineMatchId !== match?.matchId
  ) {
    startOnlineGameFromSnapshot(match, lobby);
    return;
  }
  const mine =
    match.players?.[String(onlineSeat)] || match.players?.[onlineSeat];
  const theirs =
    match.players?.[String(1 - onlineSeat)] || match.players?.[1 - onlineSeat];
  if (!mine || !theirs) return;
  const boardKey = (board) =>
    board
      ...
      .map((cell) =>
        cell
          ? `${cell.id}:${cell.type}:${cell.special || ""}:${cell.power || ""}`
          : "_",
      )
      .join("|");
  ...
  const previousRival = {
    ...
    money: bonusMoneyFor("rival"),
    time: onlineHudRivalTime ?? state.rivalTime,
    crown: crownLevelFor("rival"),
  ...
  if (!busy.player && Number(mine.seq || 0) > Number(state.onlineSeq || 0)) {
    ...
    state.playerBonusMoney = Number(
      mine.bonusMoney ?? state.playerBonusMoney ?? 0,
    );
    state.playerTime = Number(mine.time ?? state.playerTime);
    state.playerCrownBoost = Number(
      mine.crownBoost ?? state.playerCrownBoost ?? 0,
    );
    state.playerCrownLevel = Number(
      mine.crownLevel ?? state.playerCrownLevel ?? 0,
    );
    state.playerClockFreeze = Number(
      mine.clockFreeze ?? state.playerClockFreeze ?? 0,
    );
  ...
  if (state.format === "race") {
    ...
    if (serverMineTime < state.playerTime - 0.75) {
      ...
      queueHudReward("player", "time", correction, {
        label: `−${formatSeconds(-correction)}`,
      });
    }
  }
  const previousPenalty = turnPenaltyFor("player");
  state.playerTurnPenalty = Math.max(
    state.playerTurnPenalty || 0,
    Number(mine.turnPenalty || 0),
  );
  ...
  state.rivalBonusMoney = Number(
    theirs.bonusMoney ?? state.rivalBonusMoney ?? 0,
  );
  state.rivalCrownBoost = Number(
    theirs.crownBoost ?? state.rivalCrownBoost ?? 0,
  );
  state.rivalCrownLevel = Number(
    theirs.crownLevel ?? state.rivalCrownLevel ?? 0,
  );
  state.rivalClockFreeze = Number(
    theirs.clockFreeze ?? state.rivalClockFreeze ?? 0,
  );
  state.rivalTurnPenalty = Number(
    theirs.turnPenalty ?? state.rivalTurnPenalty ?? 0,
  );
  state.rivalDone = !!theirs.done;
  if (state.format === "race")
    state.rivalTime = Number(theirs.time ?? state.rivalTime);
  ...
  if (state.format === "turns")
    state.turnTimeLeft = Number(
      match.turnTimeLeft ?? state.turnTimeLeft ?? TURN_SECONDS,
    );
  state.currentSide =
    Number(match.currentSeat || 0) === onlineSeat ? "player" : "rival";
  showPlayFeedback("rival", {
    ...
    seconds:
      state.format === "race"
        ? Math.max(0, state.rivalTime - previousRival.time)
        : 0,
    moves:
      state.format === "turns" &&
      previousRival.side === "rival" &&
      state.currentSide === "rival" &&
      previousRival.round === state.currentRound
        ? Math.max(0, state.movesLeft - previousRival.moves)
        : 0,
  });
  if (crownLevelFor("rival") > previousRival.crown) {
    queueHudReward(
      "rival",
      "crown",
      crownLevelFor("rival") - previousRival.crown,
    );
  }
  renderHud();
  if (!busy.player && previousPlayerBoard !== boardKey(state.board))
    renderBoard("player");
...
async function commitOnlinePlayerIfNeeded(side = "player", extra = {}) {
  ...
  const payload = {
    ...
    powerAudioEvents: { ...state.powerAudioEvents },
      ...
      state.format === "race" ? state.playerTime <= 0 && !busy.player : false,
  };
  const updates = {
  ...
  if (state.format === "turns" && state.currentSide === "player") {
    updates["match.turnTimeLeft"] = Math.max(
      0,
      Number(state.turnTimeLeft) || 0,
    );
  }
  try {
...
async function setOnlineTurnState({
  currentSeat,
  currentRound,
  movesLeft,
  turnTimeLeft = state?.turnTimeLeft ?? TURN_SECONDS,
  status = "playing",
}) {
  const ref = onlineRoomRef();
  if (!ref) return;
...
async function toggleOnlineReady() {
  ...
  if (!Number.isInteger(onlineSeat)) {
    setOnlineMessage(
      "Escolha “Jogador 1” ou “Jogador 2” antes de ficar pronto.",
      "error",
    );
    return;
  }
  ...
  if (!ref) {
    setOnlineMessage(
      "A sala ainda não está pronta. Tente novamente em um instante.",
      "error",
    );
    return;
  }
...
function connectOnlineRoom() {
  ...
  if (!onlineConnectPromise)
    onlineConnectPromise = connectOnlineRoomOnce().finally(() => {
      onlineConnectPromise = null;
    });
...
async function connectOnlineRoomOnce() {
  ...
  if (!firebaseReady) {
    setOnlineMessage(
      "Não consegui carregar o Firebase. O restante do jogo continua funcionando offline.",
      "error",
    );
    return;
  }
  ...
  onlineUnsubscribe = fbOnSnapshot(
    ref,
    (snap) => {
      ...
      if (ready.every(Boolean))
        ...
        setOnlineMessage("Um jogador está pronto. Aguardando o outro…");
      else
        setOnlineMessage("Compartilhe o link e escolha quem é cada jogador.");
      if (
      ...
      if (
        onlineSeat === 0 &&
        ready.every(Boolean) &&
        (!data.match || data.match.status === "idle")
      ) {
    ...
    (error) => {
      ...
      setOnlineMessage(
        "Erro ao conectar no Firebase. Confira a conexão/regras da sala.",
        "error",
      );
    },
  );
...
function parseOpponent(value) {
  ...
  return {
    opponentType: "bot",
    botProfile: BOT_PROFILES[profile] ? profile : "jade",
  };
}
...
function startGame(config = {}) {
  ...
  const opponent = config.opponentType
    ? {
        ...
        botProfile: config.botProfile || "jade",
      }
  ...
  const duration = Math.max(
    15,
    Math.min(180, Number(config.duration || $("durationSelect").value || 60)),
  );
  const rounds = Math.max(
    1,
    Math.min(9, Number(config.rounds || $("roundsSelect").value || 3)),
  );
  const movesPerTurn = Math.max(
    1,
    Math.min(9, Number(config.movesPerTurn || $("movesSelect").value || 3)),
  );
  const pointValueConfig = Math.max(
    0,
    Number(
      config.pointValue ?? $("pointValueSelect").value ?? DEFAULT_POINT_VALUE,
    ),
  );
...
function resultPayload() {
  ...
  const scoreDifference = hasRival
    ? state.playerScore - state.rivalScore
    : state.playerScore;
  const playerMoneyTotal =
    Math.round(
      (scoreMoney(state.playerScore) + bonusMoneyFor("player")) * 100,
    ) / 100;
  const rivalMoneyTotal =
    Math.round((scoreMoney(state.rivalScore) + bonusMoneyFor("rival")) * 100) /
    100;
  const moneyDelta =
    Math.round(
      (hasRival ? playerMoneyTotal - rivalMoneyTotal : playerMoneyTotal) * 100,
    ) / 100;
  return {
    game: "jewels",
  ...
  if (state.opponentType === "online" && onlineRoomId && fbUpdateDoc) {
    ...
    if (ref) {
      void fbUpdateDoc(ref, {
        ...
        console.error("[Online] Falha ao encerrar partida:", error),
      );
    }
  }
...
function adjacentFromSwipe(start, dx, dy) {
  ...
  if (Math.abs(dx) > Math.abs(dy))
    return { r: start.r, c: start.c + (dx > 0 ? 1 : -1) };
...
function bindBoardInput(side) {
  ...
  root.addEventListener("pointerup", (event) => {
    ...
    const target = adjacentFromSwipe(
      gesture.pos,
...
function syncMenu(pushOnline = true) {
  ...
  $("durationLabel").textContent = isSolo
    ? "Tempo da partida"
    : "Tempo por jogador";
  ...
  if (isSolo) {
    ...
    if (startText)
      startText.textContent =
        Number.isInteger(onlineSeat) && onlineLobby?.ready?.[onlineSeat]
          ? "CANCELAR PRONTO"
          : "FICAR PRONTO";
  ...
  setHudText(
    $("menuSummary"),
    `${race ? "⏱ Corrida" : "🎯 Turnos"} · ${configText}`,
  );

  if (isOnline && pushOnline && onlineRoomId && !onlineSyncingMenu)
    queueOnlineLobbyPush({ resetReady: true });
}
...
function placeDevPreset(preset) {
  ...
  setDevStatus(
    `${side === "player" ? "Você" : "Rival"} · linha ${r + 1}, coluna ${c + 1}: ${label}`,
    "ok",
  );
  if ($("comboLabel"))
    $("comboLabel").textContent =
...
async function executeDevPiece() {
  ...
  if (!cell.special && !cell.power)
    return setDevStatus(
      "Essa casa não tem especial nem poder para executar.",
      "error",
    );
  if (busy[side])
    return setDevStatus("Espere a animação atual terminar.", "error");
  ...
  const rawGained = Math.round(
    50 +
      expanded.clearSet.size * 14 +
      expanded.triggered.size * 32 +
      expanded.powerTriggered.size * 20,
  );
            ...
            : cell.power === "crown"
              ...
              : cell.power === "devil"
                ...
                : cell.power === "cash"
                  ...
                  : cell.power === "clock"
                    ? `⏳ congela ${CLOCK_FREEZE_SECONDS}s`
                    : "Peça especial";
...
if ("IntersectionObserver" in window) {
  const boardVisibility = new IntersectionObserver(
    (entries) => {
...
document.addEventListener("pointerdown", unlockMatchAudio, {
  once: true,
  passive: true,
});
document.querySelectorAll(".mode-option").forEach((button) => {
...
["onlineName0", "onlineName1"].forEach((id) => {
  $(id).addEventListener("input", () =>
    queueOnlineLobbyPush({ resetReady: false }),
  );
});
...
if (initialParams.get("game")) {
  ...
  $("opponentSelect").value = "online";
  if (Number.isInteger(onlineSeat))
    $("onlineSeatSelect").value = String(onlineSeat);
}
populateDevSelectors();
renderDeveloperMode();
document.querySelectorAll("[data-dev-preset]").forEach((button) => {
  button.addEventListener("click", () =>
    placeDevPreset(button.dataset.devPreset),
  );
});
$("devExecuteBtn")?.addEventListener("click", () => {
  ...
  startChallenge(options = {}) {
    if (resolveExternalChallenge)
      return Promise.reject(
        new Error("Já existe um desafio de joias em andamento."),
      );
    return new Promise((resolve) => {
      ...
      startGame({
        opponentType,
        botProfile: BOT_PROFILES[options.botProfile]
          ? options.botProfile
          : "jade",
        ...
        movesPerTurn: Math.max(
          1,
          Math.min(9, Number(options.movesPerTurn) || 3),
        ),
        pointValue: Math.max(
          0,
          Number(options.pointValue) || DEFAULT_POINT_VALUE,
        ),
